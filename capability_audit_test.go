...
var sessionFieldFates = map[Provider]map[string]fieldDecl{
	ProviderClaude: {
		...
		"TurnSilenceBound":     {fateLocal, "bounds this handle's WaitForResponse; no provider is told about it"},
...
func sessionMaterialises(provider Provider, field string) bool {
	...
	case ProviderCodex:
		...
		case "SandboxWritableRoots", "SandboxNetworkAccess":
			...
			// CODEX_HOME/config.toml, so that is what is checked, and a
			// private home must be demanded for them.
			if !needsSessionMCPMaterialization(req.Config) {
				return false
			}
			toml := codexSandboxTOML(codexSandboxTuning{
				WritableRoots: req.Config.SandboxWritableRoots,
				NetworkAccess: req.Config.SandboxNetworkAccess,
			})
			if field == "SandboxWritableRoots" {
				return strings.Contains(toml, "/tmp/claudia-t24-gates")
			}
			return strings.Contains(toml, "network_access = true")
		default:
			return false
		}
	case ProviderClaude:
