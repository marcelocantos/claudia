...
func LoadPlanUsage(ctx context.Context, args *PlanUsageCacheArgs) ([]PlanUsage, error) {
	...
	if fetch == nil {
		...
		// 🎯T85: this is the path that reaches vendors, and it is shared by
		// every process on the host, so it is where the request floor
		// belongs. A caller that already named a directory keeps it.
		if all.ThrottleDir == "" {
			all.ThrottleDir = dir
		}
		fetch = func(ctx context.Context) ([]PlanUsage, error) {
